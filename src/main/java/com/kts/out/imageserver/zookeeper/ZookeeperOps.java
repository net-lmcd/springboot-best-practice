package com.kts.out.imageserver.zookeeper;

import com.kts.out.imageserver.exception.ZKException;
import com.kts.out.imageserver.exception.ZookeeperNotFoundException;
...
public class ZookeeperOps {
    ...
    public synchronized String getData(String path) throws InterruptedException, KeeperException {
        ...
        stat = Optional.ofNullable(isExists(path))
                                    .orElseThrow(() -> new ZookeeperNotFoundException("Zookeeper Node가 존재하지 않습니다"));
    ...
    public synchronized String getMinKeyFromZookeeper(String path) throws KeeperException, InterruptedException{
        String whole_data = Optional.ofNullable(this.getData(path)).
                                    orElseThrow(() -> new ZookeeperNotFoundException("Zookeeper Node의 데이터가 존재하지 않습니다."));
        StringTokenizer st = new StringTokenizer(whole_data,"\n");
        String [] urlAndCount = st.nextToken().split(" ");
        if(urlAndCount.length != 2) throw new ZKException("Zookeeper Node의 데이터가 정확하지 않습니다.");
        if(Integer.valueOf(urlAndCount[1]) < 5) {//data는 정렬되어져 있음. 0번째 데이터만 확인
    ...
    public synchronized boolean update(HttpMethod httpMethod, String key, String path) throws KeeperException, InterruptedException {
        ...
        if (whole_data != null) {
            ...
            createZNode(path, (key + " " + "0").getBytes());
            return true;
        }

        try {
            conn.getZoo().setData(path, toByteArray(data_map), -1);
        } catch (Exception e) {
